...
class ObjectProcessor(ObjectMasker):
    ...
    def object_to_df(zipped, prefix='') -> pd.DataFrame:
        ...
        # For each field in the record, extract it as a flat array
        out = {}
        for field in zipped.fields:
            # Will automatically assign None if parent is None
            out[f"{prefix}{field}"] = ak.to_numpy(zipped[field])
